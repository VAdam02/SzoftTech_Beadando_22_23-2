...
{
	public class HighSchoolBuildingTile : Tile
	{
		...
		private static Material _rim;
		public static Material Rim { get { if (_rim == null) _rim = LoadMaterialByName("Rim"); return _rim; } }
		private Material _sharedRim;
		private Material SharedRim
		{
			get
			{
				if (_sharedRim == null)
				{
					_sharedRim = new Material(Rim);
					_materials.Add(_sharedRim);
				}
				return _sharedRim;
			}
		}
        ...
        {
            GameObject popup = Instantiate(Resources.Load<GameObject>("Tiles/HighSchoolBuildingTile/HighSchoolBuildingTilePopUp"), GameObject.Find("Canvas").transform);
		...
		{
			return Resources.Load<Material>("Tiles/HighSchoolBuildingTile/Material/" + name);
		}
		...
		private void SetSharedMaterials(Renderer renderer)
		{
			...
			for (int i = 0; i < materials.Length; i++)
			{
				...
				else if (materials[i].name.Split(' ')[0] == "Rim")	{ materials[i] = SharedRim; }
